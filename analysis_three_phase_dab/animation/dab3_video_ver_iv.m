...
colore1 = [0.95 0.85 0.30];   % giallo caldo   – u1 primario
colore2 = [0.25 0.85 0.90];   % ciano          – u2 secondario
colore3 = [0.95 0.55 0.20];   % arancio tenue  – i1 trasformatore
...
colore4 = [0.45 0.85 0.55];   % verde salvia   – V output
colore5 = [0.75 0.60 0.95];   % lilla          – I output
...
V_max = max([abs(segnale_1); abs(segnale_2)]) * 1.5;
...
h_line_1 = plot(ax1, NaN, NaN, 'Color',colore1, 'LineWidth',tratto2);
h_line_2 = plot(ax1, NaN, NaN, 'Color',colore2, 'LineWidth',tratto2);
...
hold(ax1r,'on');
I1_max = max(abs(segnale_3)) * 1.5;
ylim(ax1r, [-I1_max  I1_max]);
ylabel(ax1r,'Current  [A]','Color',colore3,'FontSize',fontsize_axis);

h_line_3 = plot(ax1r, NaN, NaN, 'Color',colore3, 'LineWidth',tratto2);
...
% -----------------------------------------------------------------------
%  SUBPLOT 212 – intera simulazione: V_out (y-sx) + I_out (y-dx)
%  finestra FISSA [t_start  t_end], cursore verticale mobile
% -----------------------------------------------------------------------
...
V4_max = max(abs(segnale_4)) * 1.5;
ylim(ax2, [0  V4_max]);
xlim(ax2, [t_start  t_end]);
xlabel(ax2,'Time  [s]','Color','w','FontSize',fontsize_axis);
ylabel(ax2,'Output Voltage  [V]','Color',colore4,'FontSize',fontsize_axis);
title(ax2,'Three Phase DAB – Output Voltage & Current  (full simulation)', ...
      'Color','w','FontSize',fontsize_title);

% traccia intera simulazione (statica) per V e I
plot(ax2, tempo, segnale_4, 'Color',[colore4 0.35], 'LineWidth',tratto1);

h_line_4 = plot(ax2, NaN, NaN, 'Color',colore4, 'LineWidth',tratto3);
h_vline2 = xline(ax2, t_start, '-', 'Color','w', 'LineWidth',1.5, 'Alpha',0.8);

% asse y destra – corrente uscita
ax2r = axes('Position', ax2.Position, 'Color','none', ...
            'XColor','none', 'YColor',colore5, ...
            'FontSize',fontsize_axis, 'LineWidth',tratto2, ...
            'YAxisLocation','right', 'XLim',[t_start  t_end]);
hold(ax2r,'on');
I5_max = max(abs(segnale_5)) * 1.5;
ylim(ax2r, [-I5_max  I5_max]);
ylabel(ax2r,'Output Current  [A]','Color',colore5,'FontSize',fontsize_axis);

% traccia intera simulazione (statica) per I
plot(ax2r, tempo, segnale_5, 'Color',[colore5 0.35], 'LineWidth',tratto1);
...
for k = 1:N_frames
    ...
    % estrai anche segnali 4 e 5 (traccia progressiva fino a tc)
    mask_full     = (tempo <= tc);
    segnale_4_seg = segnale_4(mask_full);
    segnale_5_seg = segnale_5(mask_full);
    tempo_full    = tempo(mask_full);
    ...
    % --- aggiorna ax2 (y-sx V_out) e ax2r (y-dx I_out) – finestra fissa ---
    set(h_line_4, 'XData', tempo_full, 'YData', segnale_4_seg);
    set(h_line_5, 'XData', tempo_full, 'YData', segnale_5_seg);
    h_vline2.Value = tc;
